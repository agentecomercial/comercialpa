...
(function(){
  ...
  /* Estrutura inicial — categorias de regras (placeholder, a preencher) */
  var SECOES = [
    { id:'comissoes',  emoji:'💰', cor:'251,146,60',  titulo:'Política de Comissões',     sub:'Percentuais, gatilhos de pagamento e regras por produto.' },
    ...
    { id:'conduta',    emoji:'⚖️', cor:'244,114,182', titulo:'Conduta Comercial',          sub:'Ética, postura e regras de relacionamento com o cliente.' },
    { id:'posvenda',   emoji:'🤝', cor:'45,212,191',  titulo:'Pós-venda e Carteira',       sub:'Distribuição de leads, recompra e gestão da carteira.' }
  ];
  ...
  function _injectCss(){
    ...
    var css = ''
      ...
      + '.rgc-spacer{ flex:1; }'
      + '.rgc-body{ max-width:1080px; margin:0 auto; padding:26px 22px 60px; }'
      ...
      + '.rgc-card-tit{ font-size:17px; font-weight:800; }'
  ...
  function _gridHtml(){
    var cards = SECOES.map(function(s){
      return ''
        + '<button class="rgc-card" style="--c:'+s.cor+';" onclick="window._showToast && window._showToast(\'Seção \\\''+s.titulo+'\\\' — conteúdo a definir.\', \'var(--muted)\')">'
    ...
    return ''
      ...
      + '<div class="rgc-note">⚙️ <b>Estrutura inicial.</b> Defina o conteúdo de cada seção: comissões, metas, descontos, processo de venda, conduta e pós-venda. É só me dizer o que entra em cada uma.</div>'
  ...
  function _montarShell(){
    ...
    host.innerHTML = ''
      ...
      +   '<button class="rgc-back" onclick="window.voltarHomeRegras()">‹ Voltar</button>'
      +   '<div class="rgc-title">📋 Regras Comerciais</div>'
      +   '<div class="rgc-spacer"></div>'
      + '</div>'
      + '<div class="rgc-body">' + _gridHtml() + '</div>';
  }
